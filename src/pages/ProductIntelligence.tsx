...
import { productDetails, fmtCurrency, fmtDate, fmtNum } from "@/lib/analytics";
...
export default function ProductIntelligence() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="ذكاء المنتجات" subtitle="تحليل دورة حياة كل منتج: تكرار الطلب، أزمنة التنفيذ، حركة السعر، وحالة النشاط." />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Section title="المنتجات">
          ...
          <div className="max-h-[500px] overflow-y-auto text-sm space-y-0.5">
            {filtered.map((p) => (
              <button key={p}
                className={`w-full text-right px-3 py-2 rounded-md ${selected === p ? "bg-brand-navy text-primary-foreground" : "hover:bg-muted"}`}
          ...
          {d && selected ? (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <KpiCard label="آخر طلب" value={fmtDate(d.last)} />
                    ...
                    <tbody>
                      {[...d.tx].reverse().map((t) => (
                        <tr key={t.id} className="border-b border-border/40">
                          ...
                          <td className="py-2 px-2"><span className="num">{fmtNum(t.quantity)}</span></td>
                          <td className="py-2 px-2"><span className="num">{fmtCurrency(t.unitPrice)}</span></td>
